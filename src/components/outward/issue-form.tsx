...
export function IssueForm({ items, reasons }: { items: Item[]; reasons: Reason[] }) {
  ...
  const source = useMemo(() => item?.locationStocks.find((l) => l.id === locationStockId), [item, locationStockId]);
  const sourceAvailable = source ? Math.max(0, source.quantity - source.reservedQty) : 0;

  async function submit() {
    setMsg(null);
    if (!itemId) return setMsg({ ok: false, text: "Select an item." });
    if (!locationStockId) return setMsg({ ok: false, text: "Select exact source location." });
    setBusy(true);
    try {
      const res = await fetch("/api/stock/issue", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ itemId, locationStockId, reason, quantity, party, reference, note }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Could not issue.");
      setMsg({ ok: true, text: `Issued successfully. Total stock: ${data.newStock} · Source location: ${data.locationQty}.` });
  ...
  return (
    <div className="rounded-2xl border border-slate-200/70 bg-white p-5 shadow-card dark:border-white/10 dark:bg-[rgb(var(--surface))]">
      <h3 className="mb-4 font-display text-sm font-semibold text-ink dark:text-slate-100">Issue material from exact location</h3>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <F label="Item" span>
          <select value={itemId} onChange={(e) => { setItemId(e.target.value); setLocationStockId(""); }} className={inputCls}>
            <option value="">— Select item —</option>
            {items.map((i) => <option key={i.id} value={i.id}>{i.name} ({i.sku}) · {i.currentStock} total</option>)}
          </select>
        </F>

        <F label="Source location" span>
          <select value={locationStockId} onChange={(e) => setLocationStockId(e.target.value)} className={inputCls} disabled={!item}>
            <option value="">— Select exact location —</option>
            {(item?.locationStocks || []).map((l) => {
              const label = [l.warehouse.name, l.rack?.name, l.shelf?.name, l.bin?.name].filter(Boolean).join(" · ");
              const available = Math.max(0, l.quantity - l.reservedQty);
              return <option key={l.id} value={l.id}>{label} · Available {available}</option>;
            })}
          </select>
          {item && item.locationStocks.length === 0 && <span className="mt-1 flex items-center gap-1 text-xs text-amber-600"><MapPin className="h-3 w-3" /> No location stock yet. Add/assign stock through Inward first.</span>}
        </F>
        ...
        <F label={`Quantity ${source ? `(max ${sourceAvailable})` : ""}`}>
          <input type="number" min={0.001} step="any" max={sourceAvailable || undefined} value={quantity} onChange={(e) => setQuantity(Number(e.target.value))} className={inputCls} />
        </F>
