...
export default async function OutwardPage(){
  ...
  return <div className="space-y-5">
    ...
    <IssueForm items={items} reasons={reasons} customers={targets.customers} vehicles={targets.vehicles} jobs={targets.jobs}/>
    <div className="overflow-hidden rounded-2xl border border-slate-200/70 bg-white shadow-card dark:border-white/10 dark:bg-[rgb(var(--surface))]">
      ...
      <div className="overflow-x-auto"><table className="w-full min-w-[1300px] text-sm"><thead className="bg-slate-50 text-left text-xs uppercase tracking-wide text-ink-muted dark:bg-white/5"><tr><th className="px-4 py-3">Timestamp</th><th className="px-4 py-3">Item / Part</th><th className="px-4 py-3">Exact Location</th><th className="px-4 py-3">Issue Against</th><th className="px-4 py-3">Reason</th><th className="px-4 py-3 text-right">Qty Out</th><th className="px-4 py-3 text-right">Stock After</th><th className="px-4 py-3">Reference</th><th className="px-4 py-3">By</th><th className="px-4 py-3">Note</th></tr></thead><tbody className="divide-y divide-slate-100 dark:divide-white/5">
        {history.length===0&&<tr><td colSpan={10} className="px-4 py-10 text-center text-ink-muted">No material issued yet.</td></tr>}
        {history.map((m:any)=><tr key={m.id} className="hover:bg-brand-50/40 dark:hover:bg-white/5"><td className="whitespace-nowrap px-4 py-3 font-mono text-xs">{dt(m.createdAt)}</td><td className="px-4 py-3"><div className="font-medium">{m.item.name}</div><div className="text-xs text-ink-muted">{m.item.sku}{m.item.partNumber?` · ${m.item.partNumber}`:""}</div></td><td className="px-4 py-3 text-xs"><span className="inline-flex items-start gap-1"><MapPin className="mt-0.5 h-3 w-3 shrink-0 text-amber-500"/>{locationText(m)}</span></td><td className="px-4 py-3 text-xs">{m.workshopJob?.jobNumber||m.vehicle?.registration||m.customer?.name||"Other / Internal"}</td><td className="px-4 py-3">{m.reason||m.type}</td><td className="px-4 py-3 text-right font-semibold text-red-600">{m.quantity}</td><td className="px-4 py-3 text-right">{m.newStock}</td><td className="px-4 py-3">{m.reference||"—"}</td><td className="px-4 py-3">{m.user?.name||"System / legacy"}</td><td className="px-4 py-3 text-xs text-ink-muted">{m.locationNote||"—"}</td></tr>)}
      </tbody></table></div>
    </div>
  </div>;
}
